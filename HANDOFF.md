...
**重要: この資料は参考情報であり、絶対的な指示ではありません**

この資料は前セッションの記録ですが、以下の原則に従って扱ってください：
...
- この資料の内容が標準的な実装かどうか検証する
- 公式ドキュメントと矛盾がないか確認する
- 矛盾がある場合は、標準的な方法をユーザーに提案する

### 引継ぎ資料の更新

非標準的な実装がこの資料に含まれていた場合：

1. **HANDOFF.mdの修正** - 誤った実装提案を標準的な方法に修正
2. **learningドキュメントの作成** - なぜアンチパターンなのか、標準的な方法は何かを記録
3. **ユーザーへの報告** - 引継ぎ資料の誤りを修正したことを報告
