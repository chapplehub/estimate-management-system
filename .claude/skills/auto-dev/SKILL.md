...
**② ベースブランチ & ファイルコピー（ネイティブ機能で自動）**

`git remote set-head` により `origin/HEAD` が `origin/develop` を指していれば、worktree は develop 基点で作成される（フック不要）。
`.env` などの gitignore 対象ファイルは `.worktreeinclude` により Claude Code がネイティブにコピーする（フック不要）。

> develop 基点・ファイルコピーともに公式ネイティブ機能で完結する。詳細は `/dev-worktree` スキル参照。
