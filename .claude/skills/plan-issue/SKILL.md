...
1. 各 step を GitHub 形式のチェックボックス（`- [ ]`）で列挙する
2. ファイル先頭に、以下の「実行規約」注記を必ず含める（実装セッションが
   CLAUDE.md を読まなくても自己完結するため）:

   > **実行規約**: 各 step 完了時にこのファイルの該当チェックボックスを
   > `- [x]` に更新し、その更新を step のコミットに含めること。
   > 再開時（compaction 後・新規セッション後を含む）は、まずこのファイルを
   > 読み、未チェック（`- [ ]`）の最小 step 番号から続行すること。
