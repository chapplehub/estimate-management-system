...
/**
 * 納品先別販売単価 CRUD/権限（UC-3/4/5/6＋ドメインエラー＋権限）の E2E。
 *
 * DB 変更系は専用納品先 D904 × 専用商品（PRD877=Chain A・PRD878=Chain B・PRD879=Chain C）に隔離し、
 * 関心ごとに serial chain へ分割する（ADR-0020 判断1）。隔離の自然な単位は商品ではなく納品先で、
 * 閲覧系 D902 帯とは物理的に分離する。入力日付は today（JST 暦日）相対で生成し、新規期間は不変条件上
 * 開始 ≥ 今日とする（ADR-20260629-3x5）。重複拒否は登録が拒否され DB を変えないため閲覧系 D904×PRD876
 * に相乗りで並列・不変として検証する。
 *
 * 得意先別販売単価 CRUD（`customer-selling-prices/customer-selling-prices-crud.e2e.ts`）の同型写像。宛先キーが
 * `(deliveryLocationCd, productCd)` 複合になる点、入力ラベルが「納品先別販売単価」になる点、上書きなし（集約なし）が
 * 未設定ではなく「得意先別→共通へフォールバックする正常状態」の文言になる点が納品先別固有の差分。
 *
 * 対象外（コメントのみ）:
 * - 楽観ロック競合（version 不一致）: UI からの誘発が困難（同一行の同時 2 操作が要る）。
 * - 状態別サーバ拒否（失効/現在有効行の編集削除など）: UI から不可達（ボタン自体が出ない）。
 *   認可・状態別不変条件の正本検証は BE/アクション層に委ねる（得意先別・共通側と同判断）。
 */
...
/** 上書きなし（集約なし）の既定文言。納品先別固有: 未設定ではなく得意先別→共通フォールバックの正常状態。 */
const NO_OVERRIDE_MESSAGE =
  "この納品先×商品の上書きはありません。価格決定は得意先別販売単価を、無ければ共通販売単価を適用します。";

test.describe.serial("納品先別販売単価 Chain A（登録→編集→削除・D904×PRD877）", () => {
  test("UC-3: 将来期間を新規登録できる", async ({ page }) => {
    ...
    // 初期は上書きなし（集約なし＝得意先別→共通へフォールバックする正常状態）。
    ...
    await page.getByLabel("納品先別販売単価（円）").fill("5000"); // 終了日は空＝無期限
  ...
  test("UC-4: 将来行の単価・適用開始日を編集できる", async ({ page }) => {
    ...
    await page.getByLabel("納品先別販売単価（円）").fill("6000");
  ...
  test("UC-5: 将来行を削除すると上書きなしに戻る", async ({ page }) => {
    ...
    // 0期間 → 上書きなしメッセージへ自己クリーン（未設定ではなく得意先別→共通フォールバックの正常状態）。
  ...
  test("#512 回帰: 最終行の削除後、詳細に留まったまま再登録できる（空集約シェルを残さない）", async ({
    ...
    await page.getByLabel("納品先別販売単価（円）").fill("7000");
    await page.locator("form").getByRole("button", { name: "登録" }).click();
...
test.describe.serial("納品先別販売単価 Chain B（登録→適用終了→改定・D904×PRD878）", () => {
  test("UC-3: 現在有効期間を新規登録できる", async ({ page }) => {
    await page.goto("/delivery-location-selling-prices/D904/PRD878");
    await waitForAdminDetailReady(page);
    ...
    await page.getByLabel("納品先別販売単価（円）").fill("4000");
    await page.locator("form").getByRole("button", { name: "登録" }).click();
  ...
  test("UC-3: 改定として新たな将来期間を追加できる", async ({ page }) => {
    ...
    await page.getByLabel("納品先別販売単価（円）").fill("4500");
    await page.locator("form").getByRole("button", { name: "登録" }).click();
...
test.describe.serial("納品先別販売単価 Chain C（ガイド付き単価改定・D904×PRD879）", () => {
  test("UC-3: 改定の起点となる現在有効期間を新規登録できる", async ({ page }) => {
    await page.goto("/delivery-location-selling-prices/D904/PRD879");
    await waitForAdminDetailReady(page);
    await expect(page.getByText(NO_OVERRIDE_MESSAGE)).toBeVisible();
  ...
  test("UC-6: 改定ボタンから現在有効単価を改定日で新単価へ切り替える", async ({ page }) => {
    ...
    // 現単価が読み取り表示される（納品先別ラベル）。
    await expect(page.getByText("現在の納品先別販売単価")).toBeVisible();

    const revisionDate = jstRelativeDate(30);
    await page.getByLabel("改定日").fill(revisionDate);
    await page.getByLabel("改定後の納品先別販売単価（円）").fill("5000");
...
test.describe("納品先別販売単価 ドメインエラー（重複拒否・D904×PRD876・DB不変）", () => {
  test("現在有効期間に重なる登録はフォームエラーで拒否される", async ({ page }) => {
    // 閲覧系 D904×PRD876 の現在有効上書き [today-30, today+30) に today 開始で重ねる。
    ...
    await page.getByLabel("納品先別販売単価（円）").fill("9999");
    await page.locator("form").getByRole("button", { name: "登録" }).click();
...
test.describe("納品先別販売単価（一般ユーザー・権限）", () => {
