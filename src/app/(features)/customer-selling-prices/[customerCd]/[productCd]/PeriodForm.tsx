...
type Props = {
  /** コマンド宛先キー（編集読みモデルが返す customerId を bind）。 */
  customerId: string;
  /** コマンド宛先キー（編集読みモデルが返す productId を bind）。 */
  productId: string;
  /** route／revalidate 用キー（編集読みモデルが返す customerCode を bind）。 */
  customerCode: string;
  ...
  period?: CustomerSellingPriceEditPeriodDTO;
  /** 成功時（パネルを閉じる）。 */
  onSuccess: () => void;
  /** キャンセル時（パネルを閉じる）。 */
  onCancel: () => void;
...
function pickRuntime(
  ...
  switch (mode) {
    case "new":
      return {
        action: addPeriodAction.bind(null, customerId, productId, customerCode, productCode),
    ...
    case "endDate":
      return {
        action: endDatePeriodAction.bind(null, customerId, productId, customerCode, productCode),
...
export function PeriodForm({
  ...
  const runtime = pickRuntime(mode, customerId, productId, customerCode, productCode);
      ...
      <form {...getFormProps(form)} noValidate className="space-y-4">
        ...
        {mode === "endDate" && period != null && (
          <dl className="grid grid-cols-2 gap-4 bg-gray-50 rounded p-4">
            ...
            <div>
              <dt className="text-sm font-bold text-gray-700">得意先別販売単価</dt>
