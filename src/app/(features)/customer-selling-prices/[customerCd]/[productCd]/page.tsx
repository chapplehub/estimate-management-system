...
export default async function CustomerSellingPriceDetailPage({
  params,
}: {
  params: Promise<{ customerCd: string; productCd: string }>;
}) {
  const session = await verifySession();
  const admin = isAdmin(session);
  const { customerCd, productCd } = await params;
        ...
        <dl className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <dd className="mt-1 flex items-center gap-2 text-gray-900">
              {detail.customerName}
              {!detail.customerIsActive && <Badge variant="outline">無効</Badge>}
            </dd>
          </div>
        </dl>
      </div>
