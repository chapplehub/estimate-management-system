 ...
 *
 * 共通販売単価（`common-selling-prices/[productCd]/ReviseForm.tsx`）の同型写像で、宛先キーが
 * `(customerId, productId)` 複合になる点だけが異なる。
 */
export function ReviseForm({
  ...
  const { form, fields, isPending } = useServerForm({
    action: revisePeriodAction.bind(null, customerId, productId, customerCode, productCode),
    ...
    <div className="bg-white border rounded px-6 pt-5 pb-6 mt-4">
      ...
      <form {...getFormProps(form)} noValidate className="space-y-4">
        ...
        <dl className="bg-gray-50 rounded p-4">
          <dt className="text-sm font-bold text-gray-700">現在の得意先別販売単価</dt>
