...
import { deleteCustomerSellingPricePeriodCommandFactory } from "@subdomains/pricing/application/factories/deleteCustomerSellingPricePeriodCommandFactory";
import { editCustomerSellingPricePeriodCommandFactory } from "@subdomains/pricing/application/factories/editCustomerSellingPricePeriodCommandFactory";
import { endDateCustomerSellingPricePeriodCommandFactory } from "@subdomains/pricing/application/factories/endDateCustomerSellingPricePeriodCommandFactory";
import { registerCustomerSellingPricePeriodCommandFactory } from "@subdomains/pricing/application/factories/registerCustomerSellingPricePeriodCommandFactory";
import { reviseCustomerSellingPricePeriodCommandFactory } from "@subdomains/pricing/application/factories/reviseCustomerSellingPricePeriodCommandFactory";
...
/**
 * 得意先別販売単価 適用期間の操作別 Server Action（UC-3/4/5・#505 実BE接続）。
 *
 ...
 *
 * 共通販売単価（`common-selling-prices/[productCd]/actions.ts`）の同型写像で、宛先キーが
 * `(customerId, productId)` 複合になる点だけが異なる（ADR-20260627-a5c）。宛先キー（コマンド宛先）と
 * `customerCode`/`productCode`（route／revalidate 用）は編集読みモデルが返した値を `.bind()` で渡す
 * （フォーム改竄で別の得意先×商品を操作できないようにする）。価格は10進文字列で運ぶ（ADR-0022）。
 * 参照日は各 action でサーバー生成して注入する（ADR-20260627-86b）。
 */

/** 成功後に詳細＋一覧の両パスを再検証する（一覧の現在有効単価・ステータスも動くため）。 */
function revalidateBoth(customerCode: string, productCode: string): void {
  revalidatePath(`/customer-selling-prices/${customerCode}/${productCode}`);
  revalidatePath("/customer-selling-prices");
}
...
export async function addPeriodAction(
  ...
  try {
    await registerCustomerSellingPricePeriodCommandFactory().execute({
      customerId,
      productId,
      start: startDate,
      end: endDate ?? null,
    ...
    revalidateBoth(customerCode, productCode);
  } catch (error) {
...
export async function updateFuturePeriodAction(
  ...
  try {
    await editCustomerSellingPricePeriodCommandFactory().execute({
      customerId,
      productId,
      periodId,
      start: startDate,
...
export async function endDatePeriodAction(
  ...
  try {
    await endDateCustomerSellingPricePeriodCommandFactory().execute({
      customerId,
      productId,
      periodId,
      endDate,
...
export async function revisePeriodAction(
  ...
  try {
    await reviseCustomerSellingPricePeriodCommandFactory().execute({
      customerId,
      productId,
      revisionDate,
      price: String(price),
...
export async function deletePeriodAction(
  ...
  try {
    await deleteCustomerSellingPricePeriodCommandFactory().execute({
      customerId,
      productId,
      periodId,
      referenceDate: toJstCalendarDay(new Date()),
