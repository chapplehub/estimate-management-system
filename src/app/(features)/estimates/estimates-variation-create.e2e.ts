...
test.describe.serial("バリエーション複製・新規追加（S6・C3 / N9905003）", () => {
  test("提出区分を選んで新規追加→空バリ保存→新タブが自動選択される（#370）", async ({ page }) => {
    await page.goto(`/estimates/${SOURCE}`);
    ...
    // 追加成功は専用フラッシュ文言（#370）。
    await expect(page.getByText("バリエーションを追加しました。")).toBeVisible();
    // 新バリ(V2)タブが現れ、手動操作なしで選択状態になる（自動選択・#370）。
    const tab2 = page.getByRole("tab", { name: "バリエーション2" });
    await expect(tab2).toBeVisible();
    await expect(tab2).toHaveAttribute("aria-selected", "true");
    // 選択済みなので提出区分バッジ（納品先向け）がそのまま見える。
  ...
  test("既存バリを複製→提出区分固定でプリフィル→保存→新タブが現れる", async ({ page }) => {
    ...
    // 複製で新タブ（V3＝前テストの新規追加 V2 の次）が現れ、自動選択される（#370）。
    const tab3 = page.getByRole("tab", { name: "バリエーション3" });
    await expect(tab3).toBeVisible();
    await expect(tab3).toHaveAttribute("aria-selected", "true");
    // 複製元 V1 は不変（タブを戻すと元の明細が残る）。
