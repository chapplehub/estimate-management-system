 ...
 * 同一見積内に得意先宛の改訂先バリエーションを生成する。改訂先の掛率・メモは改訂元から複写、見積単価は
 * 得意先宛で再解決（#431）、固定値引（明細値引・全体値引）はクリア（ADR-20260714-pv8・#598）と
 * ドメインが決定するため利用者の入力面はなく、UI は「ボタン → 確認 → 純粋コマンド送信」に徹する。
 *
 ...
 * - (5) 改訂先（V3）は「価格を調整」だけ出る。掛率・明細値引・全体値引・メモを編集でき、粗利が
 *       ライブ反映され、保存後に閲覧へ反映される。単価は価格決定で確定・固定＝読み取り専用（#430・
 *       ADR-0064）。数量・商品も read-only で行追加削除 UI は無い（#390）
 *
...
test.describe.serial("得意先改訂（C7 / N9905006）", () => {
  ...
  test("改訂先の価格調整フォームは数量・商品 read-only・行追加削除 UI 無しで、価格と全体値引・メモを編集して保存できる（#390）", async ({
    ...
    // 単価は価格決定で確定・固定＝読み取り専用（#430・ADR-0064）。編集入力は出ない（表示のみ）。
    await expect(page.getByRole("textbox", { name: /^単価（/ })).toHaveCount(0);

    // 掛率を下げると行金額が下がり合計粗利がライブで変わる（保存前）。単価は据え置きのまま調整は
    // 掛率・明細値引・全体値引に限定される（#430）。総額は正のままで保存できる。
    const grossBefore = await page.getByLabel("合計粗利").textContent();
    const discountRate = page.getByLabel(/^掛率（/).first();
    await discountRate.fill("0.5");
