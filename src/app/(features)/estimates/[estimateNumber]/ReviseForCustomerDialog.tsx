 ...
 * 掛率・メモは改訂元から複写、見積単価は得意先宛で再解決（#431）、固定値引（明細値引・全体値引）は
 * クリア（ADR-20260714-pv8・#598）とドメインが決定するため
 * 利用者の入力面は無く、UI は「ボタン → 確認 →
        ...
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              この見積内に作成します。掛率・メモは改訂元から引き継ぎ、見積単価は得意先宛で
              再解決されます。
            </DialogDescription>
          </DialogHeader>
  ...
  return (
    <form {...getFormProps(form)} noValidate>
      ...
      <div className="bg-amber-50 border border-amber-300 text-amber-800 text-sm px-3 py-2 rounded mb-4 space-y-2">
        ...
        {/* 単価再解決（#431）と固定値引クリア（ADR-20260714-pv8・#598）の告知。掛率・メモは複写する。 */}
        <p>
          改訂先の見積単価は、得意先宛で価格マスタから再解決されます（改訂元の単価は引き継がれません）。
          販売単価が未設定の商品があると改訂できません。
        </p>
        <p>
          改訂元の固定値引（明細値引・全体値引）は引き継がれません。単価が得意先宛に変わるため、値引は
          改訂先で入れ直してください（掛率は引き継がれます）。
        </p>
        {!hasRevision && (
          <p>
