 ...
 *
 * DB 変更系は専用商品（PRD903=Chain A・PRD905=Chain B）に隔離し、関心ごとに serial chain へ分割する
 * （ADR-0020 判断1）。入力日付は today（JST 暦日）相対で生成し、新規期間は不変条件上 開始 ≥ 今日とする
 * （ADR-20260629-3x5）。重複拒否は登録が拒否され DB を変えないため母体 PRD901 で並列・不変として検証する。
 *
 * 対象外（コメントのみ）:
...
test.describe.serial("共通販売単価 Chain A（登録→編集→削除・PRD903）", () => {
  test("UC-3: 将来期間を新規登録できる", async ({ page }) => {
    await page.goto("/common-selling-prices/PRD903");
...
test.describe.serial("共通販売単価 Chain B（登録→適用終了→改定・PRD905）", () => {
  test("UC-3: 現在有効期間を新規登録できる", async ({ page }) => {
    await page.goto("/common-selling-prices/PRD905");
    await waitForAdminDetailReady(page);
...
test.describe("共通販売単価 ドメインエラー（重複拒否・PRD901・DB不変）", () => {
