...
/**
 * 原価 CRUD/権限（UC-3/4/5＋ドメインエラー＋権限）の E2E。
 *
 * DB 変更系は専用商品（PRD845=Chain A・PRD846=Chain B・PRD847=Chain C）に隔離し、関心ごとに
 * serial chain へ分割する（ADR-0020 判断1）。入力日付は today（JST 暦日）相対で生成し、新規期間は
 * 不変条件上 開始 ≥ 今日とする（ADR-20260629-3x5）。重複拒否は登録が拒否され DB を変えないため
 * 母体 PRD844 で並列・不変として検証する。共通販売単価 CRUD E2E の同型ミラー。
 *
 * 対象外（コメントのみ）:
...
test.describe.serial("原価 Chain A（登録→編集→削除・PRD845）", () => {
  test("UC-3: 将来期間を新規登録できる", async ({ page }) => {
    await page.goto("/cost-prices/PRD845");
    ...
    await page.getByLabel("原価（円）").fill("5000"); // 終了日は空＝無期限
  ...
  test("UC-4: 将来行の単価・適用開始日を編集できる", async ({ page }) => {
    ...
    await page.getByLabel("原価（円）").fill("6000");
...
test.describe.serial("原価 Chain B（登録→適用終了→改定・PRD846）", () => {
  test("UC-3: 現在有効期間を新規登録できる", async ({ page }) => {
    await page.goto("/cost-prices/PRD846");
    await waitForAdminDetailReady(page);
    ...
    await page.getByLabel("原価（円）").fill("4000");
    await page.locator("form").getByRole("button", { name: "登録" }).click();
  ...
  test("UC-3: 改定として新たな将来期間を追加できる", async ({ page }) => {
    ...
    await page.getByLabel("原価（円）").fill("4500");
    await page.locator("form").getByRole("button", { name: "登録" }).click();
...
test.describe.serial("原価 Chain C（ガイド付き単価改定・PRD847）", () => {
  test("UC-3: 改定の起点となる現在有効期間を新規登録できる", async ({ page }) => {
    await page.goto("/cost-prices/PRD847");
    await waitForAdminDetailReady(page);
    await expect(page.getByText("適用期間が未設定です")).toBeVisible();
  ...
  test("UC-6: 改定ボタンから現在有効単価を改定日で新単価へ切り替える", async ({ page }) => {
    ...
    // 現単価が読み取り表示される
    await expect(page.getByText("現在の原価")).toBeVisible();

    const revisionDate = jstRelativeDate(30);
    await page.getByLabel("改定日").fill(revisionDate);
    await page.getByLabel("改定後の原価（円）").fill("5000");
...
test.describe("原価 ドメインエラー（重複拒否・PRD844・DB不変）", () => {
  test("現在有効期間に重なる登録はフォームエラーで拒否される", async ({ page }) => {
    await page.goto("/cost-prices/PRD844");
    await waitForAdminDetailReady(page);
    ...
    await page.getByLabel("原価（円）").fill("9999");
    await page.locator("form").getByRole("button", { name: "登録" }).click();
