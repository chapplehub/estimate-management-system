...
/**
 * 原価 適用期間の操作別バリデーションスキーマ（UC-3/4/5・#503）。
 *
 * 単項目の形（必須・実在日・整数・非負）と、同一フォーム内で完結する交差検証
 * （終了>開始）のみをここで担保する。集約横断の重複・状態別権限・楽観ロック競合・
 * 適用終了の「本日以降」は BE コマンド（集約の不変条件）が最終判定する（#502）。
 * 操作別に分けることで、現在有効行の開始日・単価を入力契約から外し改竄不能にする。
 * 共通売単価 common-selling-prices/[productCd]/schema.ts の同型ミラー。
 */
...
/** 原価（0以上の整数。マスタ正規入力として0円を許容＝use-cases.md ルール3）。 */
 ...
 * 省略を許容する（BE の RegisterCommand が expectedVersion 省略時は create+insert を選ぶ・#502）。
...
/**
 * 単価改定（ガイド付き）: 改定日（必須・実在日）＋ 新単価。
 * 現在有効行はサーバー側で特定するため periodId は入力契約に含めない（改竄面を消す）。
