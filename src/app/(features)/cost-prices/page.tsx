...
import { costPriceListQueryFactory } from "@subdomains/pricing/application/factories/pricingQueryFactory";
import type { CostPricePriceStatus } from "@subdomains/pricing/application/queries/dto/CostPriceListItemDTO";
...
function validatePriceStatusFilter(value: string | undefined): CostPricePriceStatus | undefined {
...
export default async function CostPricePage({
  ...
  const items = await costPriceListQueryFactory().list({
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex justify-between items-center mb-2 px-4 pt-4">
        <h1 className="text-3xl font-bold">原価</h1>
      </div>
      ...
      <div className="flex-1 flex flex-col min-h-0 bg-white shadow-md rounded mx-4 mb-4 text-gray-500">
        <div className="px-8 pt-6 pb-2">
          <h2 className="text-xl font-semibold">原価一覧</h2>
        </div>
