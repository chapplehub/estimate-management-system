 ...
 * 原価 詳細（UC-2）の E2E。
 *
 * PRD844 は失効/現在有効/将来の3期間を持つ（today 相対シード・ADR-20260629-3x5）。状態バッジ
...
test.describe("原価 詳細（UC-2）", () => {
  test("商品情報と3状態バッジが表示される", async ({ page }) => {
    ...
    await expect(page.getByText("PRD844")).toBeVisible();
    await expect(page.getByText("COST_3状態テスト商品")).toBeVisible();
  ...
  test("戻りリンクから一覧へ遷移できる", async ({ page }) => {
    ...
    await page.getByRole("link", { name: "← 原価一覧に戻る" }).click();
...
/**
 * 原価 タイムライン表示（#475）の E2E。
 *
 * PRD844（失効/現在有効/将来の3期間）で、既定はテーブル（帯なし）→ トグルでタイムライン帯を付加表示
 * → テーブルへ戻すと帯が消えることを検証する。凡例（現在有効/失効/将来）はテーブルの状態バッジと
 * テキストが衝突するため data-testid でスコープする。共通シード・DB 不変（並列可）。
 * 共通販売単価 タイムライン E2E の同型ミラー（`data-testid` は売単価と同一）。
 */
test.describe("原価 タイムライン表示（#475）", () => {
