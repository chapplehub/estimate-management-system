 ...
 * イベント payload（承認者・承認日時）のみを保持する。発生日時は raw `Date` ではなく
 * 不変VO {@link OccurredAt} で保持し、イベント行の `createdAt` に対応する（ADR-20260624-8f5）。
 */
export class StepApproval {
  ...
  static create(approverEmployeeId: EmployeeId, occurredAt: OccurredAt): StepApproval {
