...
describe("StepApproval", () => {
  describe("create()", () => {
    it("承認者・承認日時を保持する", () => {
      ...
      expect(approval.occurredAt.getTime()).toBe(occurredAt.getTime());
    });
  });
  ...
  describe("equals()", () => {
    it("承認者・承認日時がともに一致すれば等価", () => {
      ...
      const a = StepApproval.create(approver, occurredAt);
      const b = StepApproval.create(approver, new Date("2026-06-19T09:00:00Z"));
    ...
    it("承認日時が異なれば非等価", () => {
      const approver = EmployeeId.generate();
      const a = StepApproval.create(approver, new Date("2026-06-19T09:00:00Z"));
      const b = StepApproval.create(approver, new Date("2026-06-19T10:00:00Z"));
