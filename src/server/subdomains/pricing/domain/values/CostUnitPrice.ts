 ...
 * 非負の不変条件を持つ。`SellingUnitPrice`（共通売単価側の金額VO）と対をなす（ADR-20260627-a5c）。
 */
export class CostUnitPrice {
   ...
   */
  static fromMoney(amount: Money): CostUnitPrice {
    if (amount.isNegative()) {
      throw new ValidationError("原価は0以上で指定してください");
    }
    return new CostUnitPrice(amount);
  }
  ...
  equals(other: CostUnitPrice): boolean {
