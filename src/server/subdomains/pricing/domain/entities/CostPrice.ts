...
export class CostPrice {
   ...
   */
  addPeriod(period: ApplicablePeriod, price: CostUnitPrice, referenceDate: string): void {
   ...
   */
  editPeriod(
    periodId: CostPricePeriodId,
    changes: { period: ApplicablePeriod; price: CostUnitPrice },
    referenceDate: string
  ): void {
    const row = this.requireRow(periodId);
    if (!this.isFuture(row.period, referenceDate)) {
      throw new BusinessRuleViolationError(
        `${CostPrice.ENTITY_NAME}は将来行のみ編集できます（現在有効・失効の行は変更不可）`
      );
    }
   ...
   * @param referenceDate 参照日（今日・JST 暦日 `"YYYY-MM-DD"`）。
   */
  endDatePeriod(periodId: CostPricePeriodId, endDate: string, referenceDate: string): void {
    const row = this.requireRow(periodId);
    if (!row.period.contains(referenceDate)) {
      throw new BusinessRuleViolationError(
        `${CostPrice.ENTITY_NAME}は現在有効行のみ適用終了できます`
      );
    }
    ...
    if (endDate <= referenceDate) {
      throw new BusinessRuleViolationError(
        `${CostPrice.ENTITY_NAME}の適用終了日は今日より後である必要があります: ${endDate} <= ${referenceDate}`
      );
    }
    if (row.period.end !== null && endDate >= row.period.end) {
      throw new BusinessRuleViolationError(
        `${CostPrice.ENTITY_NAME}の適用終了は短縮のみ可能です（既存終了日より後へは延長できません）: ${endDate} >= ${row.period.end}`
      );
    }
   ...
   * @param referenceDate 参照日（今日・JST 暦日 `"YYYY-MM-DD"`）。
   */
  deletePeriod(periodId: CostPricePeriodId, referenceDate: string): void {
    const row = this.requireRow(periodId);
    if (!this.isFuture(row.period, referenceDate)) {
      throw new BusinessRuleViolationError(
        `${CostPrice.ENTITY_NAME}は未来開始行のみ削除できます（現在有効・失効の行は削除不可）`
      );
    }
   ...
   * @param referenceDate 参照日（今日・JST 暦日 `"YYYY-MM-DD"`）。
   */
  currentValidPeriod(referenceDate: string): CostPricePeriod | undefined {
    return this._periods.find((row) => row.period.contains(referenceDate));
  }

  /** identity で期間行を引く。存在しなければ不変条件違反として投げる。 */
  private requireRow(periodId: CostPricePeriodId): CostPricePeriod {
    const row = this._periods.find((r) => r.id.equals(periodId));
    if (row === undefined) {
      throw new BusinessRuleViolationError(
        `${CostPrice.ENTITY_NAME}に指定された適用期間行が存在しません`
      );
    }
    return row;
  }
  ...
  private assertStartNotPast(start: string, referenceDate: string): void {
    if (start < referenceDate) {
      throw new BusinessRuleViolationError(
        `${CostPrice.ENTITY_NAME}の適用開始日は今日以降である必要があります: ${start} < ${referenceDate}`
      );
    }
  }
   ...
   */
  private assertNoOverlap(period: ApplicablePeriod, excludeRow?: CostPricePeriod): void {
