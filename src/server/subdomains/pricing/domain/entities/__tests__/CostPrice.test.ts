...
describe("CostPrice 集約", () => {
  ...
  describe("addPeriod — 適用期間行の追加", () => {
    ...
    it("既存期間と重複する期間は BusinessRuleViolationError", () => {
      ...
      expect(() =>
        aggregate.addPeriod(period("2026-09-01", "2026-12-01"), cost(650), today)
    ...
    it("無期限行があるとき、その開始日以降に重なる期間は弾く", () => {
      ...
      expect(() =>
        aggregate.addPeriod(period("2030-01-01", "2030-02-01"), cost(650), today)
      ).toThrow(BusinessRuleViolationError);
    });
  ...
  describe("editPeriod — 将来行の全項目編集", () => {
    ...
    it("将来行の期間と単価を差し替えられる（identity は保持）", () => {
      ...
      aggregate.editPeriod(
        id,
        { period: period("2025-08-01", "2025-11-01"), price: cost(1500) },
        today
      );
      ...
      expect(row.price.equals(cost(1500))).toBe(true);
    });
    ...
    it("現在有効行は編集できない（BusinessRuleViolationError・単価ロック）", () => {
      ...
      expect(() =>
        aggregate.editPeriod(id, { period: period("2025-07-01", null), price: cost(1500) }, today)
      ).toThrow(BusinessRuleViolationError);
      // 失敗時は元のまま
      expect(aggregate.periods[0].period.equals(period("2025-05-01", "2025-12-01"))).toBe(true);
      expect(aggregate.periods[0].price.equals(cost(1000))).toBe(true);
    });
    ...
    it("存在しない periodId は BusinessRuleViolationError", () => {
      ...
      expect(() =>
        aggregate.editPeriod(
          CostPricePeriodId.generate(),
          { period: period("2025-08-01", null), price: cost(1500) },
          today
        )
      ).toThrow(BusinessRuleViolationError);
    });
  ...
  describe("endDatePeriod — 現在有効行の適用終了", () => {
    ...
    it("現在有効行に終了日を設定でき、開始日・単価は変わらない", () => {
      ...
      expect(row.price.equals(cost(1000))).toBe(true);
      expect(row.id.equals(id)).toBe(true);
    });
  ...
  describe("isEmpty — 空集約の判定", () => {
    ...
    it("期間行が1件以上あれば false", () => {
      const aggregate = CostPrice.reconstruct(productId, [
        {
          id: CostPricePeriodId.generate(),
          period: period("2026-04-01", null),
          price: cost(600),
        },
      ]);
