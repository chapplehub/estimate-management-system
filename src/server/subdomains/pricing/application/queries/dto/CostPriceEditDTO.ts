 ...
 * 共通売単価の同型ミラー（ADR-20260627-a5c・型は集約ごとに複製し共有しない）。
 */
export type CostPricePeriodStatus = "future" | "active" | "expired";

/** 編集画面の期間行1件 DTO。 */
export interface CostPriceEditPeriodDTO {
  periodId: string;
  start: string;
  end: string | null;
  /** 10進文字列の原価（消費側で `Money.fromDecimalString`）。 */
  costPrice: string;
  status: CostPricePeriodStatus;
}

/**
 * 原価 編集画面の読みモデル DTO（#500・read 関心・productCode キー）。
 *
 * 商品 identity（id/code/name/isActive）を同梱し、FE 側の code→id 解決クエリ・商品名の二重取得を
 * 不要にする（共通売単価 #473 と同構成）。`productId` はコマンド宛先キーとしてフォームが往復させる。
 *
 * version は楽観ロックトークンで、編集フォームが往復させ保存時に `expectedVersion` として戻す
 * （ADR-0039。参照日が実行時生成なのと逆方向＝過去から運ぶ）。原価が**未設定**（集約なし）でも
 ...
 */
export interface CostPriceEditDTO {
  productId: string;
  productCode: string;
  productName: string;
  ...
  version: number | null;
  periods: CostPriceEditPeriodDTO[];
}
