...
import { ReviseCostPricePeriodCommand } from "../ReviseCostPricePeriodCommand";

const TEST_PRODUCT_CODE = "CPCMD31";
...
describe("ReviseCostPricePeriodCommand", () => {
  let command: ReviseCostPricePeriodCommand;
  ...
  beforeEach(async () => {
    ...
    command = new ReviseCostPricePeriodCommand(repository);
  ...
  it("現在有効行を改定日で終了し、改定日開始の新行を連続して追加する", async () => {
    ...
    expect(rows[0].price.equals(cost(1000))).toBe(true);
    // 新行: 改定日開始の無期限、新原価
    expect(rows[1].period.equals(period("2025-09-01", null))).toBe(true);
    expect(rows[1].price.equals(cost(1200))).toBe(true);
  });
  ...
  it("据え置き（新原価＝現原価）も改定として成立する（拒否しない）", async () => {
    await seedActivePeriod();
    ...
    expect(rows).toHaveLength(2);
    expect(rows[1].period.equals(period("2025-09-01", null))).toBe(true);
    expect(rows[1].price.equals(cost(1000))).toBe(true);
  });
  ...
  it("version は改定1回につき1度だけ上がる（後続操作が expectedVersion=2 で通り 1 で弾かれる）", async () => {
    ...
    const endDate = new EndDateCostPricePeriodCommand(repository);
