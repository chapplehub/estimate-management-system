...
4. **失敗時の呼び出し側は「操作中断・state 凍結」**。画面 state には一切触らない（ダイアログは開いたまま・古い表示は維持）。失敗を業務上の空値（例: 税率 `null` = 税率未設定）へ変換して嘘の業務状態を見せることを禁止し、同じ操作の再実行によるリトライ経路を自然に残す。
   - **細目（#633 の自動レビュー R1-1 で追加）**: 呼び出し側が「戻り値で意味を伝える」コンポーネント契約に載っている場合、素の `return;`（= `undefined`）で抜けると**その契約の別の意味に化ける**ことがある。`SelectionModal` の `onConfirm` は `undefined` を「確定成功 → 閉じる」と解釈するため、そのままでは失敗時にモーダルが閉じて検索結果・選択状態が捨てられ、state 凍結が破れる。この経路では専用 sentinel `SELECTION_ABORTED` を返して中断を伝える（ADR-20260716-r4d に第3の戻り値として追加済み）。中断 sentinel が `message` を持たないことで、決定3（通知は toast 1枚）との二重表示も避ける。
