...
> **2026-07-30 追記（#696 → ADR-20260730-0b6）**
> 残っていた 2〜4 段目（型分離 / major 分離 / ファミリー復活）も廃止し、グルーピングは `config:recommended` 内蔵プリセットへ全面委譲した。major のグループ解除は概念ごと消滅し（分離は `separateMajorMinor` 既定 true が担う）、packageRules は major ラベル付与 + node グループの 2 ルールのみになった。記述順が意味を持つ 4 段構成はここで完全に解消した。9kq の追加基準「major が連動リリースされるか」も同 ADR で 2 条件（プリセット定義に存在しない連動 + 片方だけ上げると壊れる実測）に置き換えられている。
...
> 上記の決着は、前提とした「位置の情報」「境界の情報」の大半が実測によりプリセットで表現可能（または major グループ解除の廃止で不要）と判明したため改訂した。手書きルールは node + `@types/node` のみ残して削減し、`extends` の `group:monorepos` 明記も二重 extend のため削除した。手書き追加の基準は同 ADR の 2 条件を参照。
