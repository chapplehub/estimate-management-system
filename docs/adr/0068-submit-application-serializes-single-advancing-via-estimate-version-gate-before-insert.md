...
> **解決（ADR-0069）**: #440 で本窓を **ADR-0069** が解消した。選択肢 A（submit の bump と挿入を単一トランザクションで原子化）を採用し、本 ADR の「集約またぎ tx を張らない」スタンスを**トランザクション境界の1点だけ**更新する。version 関門・兄弟チェック・TOCTOU トークン兼用は不変。副次的に、本 ADR ケース2の `EstimateApplicationPersistError`（bump 成功・insert 失敗）は原子化により発生し得なくなり撤去対象となる（エラーは `ConflictError` と正常 union の2分岐に単純化）。詳細は ADR-0069 を参照。
