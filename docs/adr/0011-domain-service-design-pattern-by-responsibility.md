...
## 補足: Policy カテゴリの追加（2026-05-23）

本 ADR は **Domain Service の内部分類**（事実確認型／ルール検証型）を定めるものとして引き続き有効。

その後、リポジトリ依存ゼロ・同期・純粋計算の規約クラスを `domain/policies/` 配下に独立カテゴリとして配置する判断を [ADR-0023](0023-domain-policies-directory-for-calculation-rules.md) で追加した。本 ADR の対象（DomainService、リポジトリ DI ありの越境チェック用途）と直交する責務カテゴリであり、両者は補完関係にある。

`domain/services/` （DomainService）と `domain/policies/` （Policy）の責務分担と判断基準は ADR-0023 を参照。
