...
| 項目 | 値 |
|---|---|
| ステータス | 採用（決定1・型ガード条項は ADR-20260717-w4d へ置換。決定2・3 は存続） |
| 起票日 | 2026-07-16 |
| 最終更新日 | 2026-07-17 |

## 改訂履歴

- 2026-07-16 初版（repriced 記述子の `Omit` ＋ 子構築の共有ビルダー一本化 ＋ 通常/改訂のビルダー分割）
- 2026-07-17 **決定1（repriced 記述子型を `Omit` で構成する機構）と「不変則の効力を `@ts-expect-error` 型ガードテストで固定する」条項を [ADR-20260717-w4d](20260717-w4d-additive-descriptor-base-over-subtractive-commonalization.md) へ置換**（#626）。記述子は減算共通化（`Omit`）をやめ、痩せた共有核を加算拡張する方式へ交換し、「Repriced」語彙は型から撤去して `Copied*` / `Revised*` に分離した。型ガードテスト（`repricedDescriptor.type.test.ts`）は削除した（網羅性 tripwire を置かない決定に伴う）。
  **決定2（子構築の共有ビルダー一本化）・決定3（`buildVariation` / `buildRevisedVariation` の分割）は存続**し、w4d もそれに依存する。本文は当時の判断の記録として書き換えていないため、決定1 の `Omit` 前提の記述（「検討した選択肢 → 不変則の表現方法」および「決定 1.」「根拠 → 型が禁止を語る」「影響」の repriced 記述子への言及）は現行実装と一致しない。機構の正本は w4d を参照すること。
