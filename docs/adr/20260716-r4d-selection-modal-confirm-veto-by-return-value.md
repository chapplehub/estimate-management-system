...
- **追記（2026-07-23・#633）**: 非業務例外（DB 障害・ネットワーク断）で確定に必要なデータを取得できなかった場合を表す第3の戻り値 `SELECTION_ABORTED`（sentinel）を、この union に**加算的に**追加した（ADR-20260723-h7r）。閉じず・理由も出さず・画面 state を凍結する枝で、既存2状態（`undefined` = 成功で閉じる / `SelectionRejection` = 閉じずに理由と原因行を表示）は不変であり、既存呼び出し元の無改修も維持される。拒否（業務上追加できない）と中断（取得そのものに失敗した）は意味が異なるため別の枝として区別する。理由を出さないのは、通知を `callReadAction` の toast 1枚に集約する方針（ADR-20260723-h7r 決定3）とモーダル内バナーが二重表示になるのを避けるため
