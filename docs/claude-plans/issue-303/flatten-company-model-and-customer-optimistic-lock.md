# Issue #303: 取引先モデルを平坦化し customer へ楽観ロックを適用する（ADR-0040 / ADR-0039） — 実装計画
...
いずれも使っておらず計測可能な利益を生んでいないと判明したため（[ADR-0040](../../adr/0040-flatten-company-subtype-inheritance-into-aggregate-tables.md)）、
...
- **採用: B**。CTI が計測可能な利益を生んでおらず、楽観ロック変換が 0 件＝埋没コスト最小の今が分岐の最良点。
  C は同時更新されるべき name 等が集約境界を跨ぎ最悪。詳細は ADR-0040。
...
- コミットメッセージ: `feat: 取引先モデルを平坦化するマイグレーションを追加する（companies廃止・ADR-0040）`
