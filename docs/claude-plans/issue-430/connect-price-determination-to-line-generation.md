...
**コンテキスト管理**: Step 5 完了後・Step 8 完了後の2箇所で、**実装を止めてユーザーにターンを返す**（`/compact` はユーザーが実行するため。モデルは自身で `/compact` を起動できない）。各ステップのコミット直後＝クリーンな作業ツリーで停止すること。**再開後（compact 直後）の最初の行動は、本計画ファイルと ADR-20260709-5ea の再 Read に固定する**。実装済みヘルパーの公開 API は実コードを再 Read して確認する。
...
> **⏸ 停止ポイント**: このステップのコミット後、実装を止めてユーザーにターンを返す（ユーザーが `/compact` を実行する）。再開後の最初の行動は本計画ファイルと ADR-20260709-5ea の再 Read に固定する。
...
> **⏸ 停止ポイント**: このステップのコミット後、実装を止めてユーザーにターンを返す（ユーザーが `/compact` を実行する）。再開後の最初の行動は本計画ファイルの再 Read に固定する。
