...
## 逸脱6: Step 9 を2コミットに分割し、E2E 改修範囲は seed 補充が主体だった（Step 9）

### 元の計画内容
Step 9 は1コミット（`test: 見積E2Eを単価自動確定前提に改修し解決不能ケースを追加`）で、作業内容は「単価入力ステップを撤去し『商品選択 → seed の販売単価から自動確定された単価・金額』のアサーションへ書き換え」「見積スペックで使う商品に today 相対の共通販売単価を紐づけ」「解決不能ケースを1本追加」。

### 実際の実装内容
Step 9 を2コミットに分割した:
- コミット1（`4c8df9f6`・`test: 見積E2E商品に共通販売単価を付与し解決不能商品を追加する`）: seed-e2e.ts に `seedEstimateSellingPrices`（PRD001/002/810/811 へ共通販売単価）と PRD830（解決不能ケース用の有効単価なし商品）を追加。
- コミット2（`548f9799`・計画のコミットメッセージ）: `estimates-revise-for-customer` の単価入力撤去＋新規 `estimates-price-unresolvable`。

計画からの実態差:
1. **販売単価の適用期間は today 相対ではなく無期限 `[2026-04-01, ∞)`** にした。seed 見積の estimateDate は 2026-04-01 固定で、today 相対の有界窓（例 `[today-30, today+30)`）では 2026-04-01 を覆えず、C3/C4/セット展開/改訂調整の対話的な明細追加が価格決定で解決不能になるため。原価の `COST_PRICE_BASE_DATE`（2026-04-01）と同起点の無期限期間で、seed 見積（2026-04-01）と C1 作成（実行日）の両方を覆う。過去開始ゆえ raw daterange insert（他フィクスチャと同型）。
2. **「金額アサーションへ書き換え」はほぼ発生しなかった**。既存の対話系スペックは元々単価を手入力しておらず（スナップショット既定 0 のまま追加し、追加明細の金額も断定していなかった）、単価入力に触れていたのは改訂先価格調整（`estimates-revise-for-customer`）の1テストのみ。そこは単価が読み取り専用化（Step 8）したため、合計粗利のライブ変化を掛率の引き下げで駆動し、単価は textbox が出ないこと（読み取り専用）を検証する形へ書き換えた。他スペックは seed 補充だけで解決可能になり無改修。
3. **複製（C6）・改訂（C7）は resolver を通さずドメインで複写する**ため seed 単価は不要で、複製スペックは無改修（当初は要改修を懸念したが不要と判明）。

### 逸脱の理由
seed の適用期間を無期限にしたのは、見積 seed の固定日付（2026-04-01）と実行日（today）の双方を単一期間で覆う必要があったため（today 相対では成立しない）。書き換え範囲が小さかったのは、既存 E2E が単価手入力に依存していなかったという事前分析どおりの結果。コミット分割は seed（フィクスチャ）とスペック（テスト本体）で関心が分かれ、回帰の切り分けを容易にするため。

## 逸脱7: Step 10 の実機確認を playwright MCP ではなく E2E テストランナーで実施（Step 10）

### 元の計画内容
Step 10 は「playwright MCP による実機確認（コミットなし）」。playwright MCP のブラウザ操作で見積フローを手動確認する想定。

### 実際の実装内容
`pnpm e2e:seed` で seed 変更（`seedEstimateSellingPrices` の SQL・PRD830）が実 DB で通ることを確認した後、`pnpm exec playwright test`（headless chromium）で影響を受ける見積スペック全34件（create/variation-create/variation-edit/set-group-edit/revise-for-customer/duplicate/price-unresolvable）を実行し全 green を確認した。

### 逸脱の理由
実際のテストランナーでの実行は、手動 MCP クリックより網羅的かつ回帰検出力が高く、Step 10 の意図（変更後のフローが実機で成立することの確認）をより厳密に満たす。実行途中で認証 setup 由来の webServer 起動タイムアウト（stale `.next`）が出たため、ユーザが `.next` を削除→dev サーバをウォームアップ起動→reuseExistingServer で再実行して解消した（メモリ「E2E認証setup失敗は.next/を消す」と整合）。メモリ「ローカルでE2E全体を回さない」に沿い、全 spec ではなく #430 の変更に関係する見積 spec のみに絞って実行した。
