...
A（デッドコード）→ B（RANK_MAP）→ C（JSDoc）→ D（from削除）→ E（テスト追加）
...
- **#11** `approve`/`reject` の二重 `findStep` 解消（効率・内部リファクタ。公開API `stepStatus(stepId)` 維持が制約）→ [#406](https://github.com/chapplehub/estimate-management-system/issues/406)
