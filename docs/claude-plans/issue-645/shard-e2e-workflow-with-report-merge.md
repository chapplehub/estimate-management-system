...
#### 実測結果（run 30241646344 / PR #653）

ウォールクロック **4 分 50 秒**（06:06:03 → 06:10:53）。shard 化前の 9 分 33 秒から **4 分 43 秒短縮（49%）**。

| ジョブ | 所要 |
|---|---|
| e2e (shard 1/4) | 3 分 36 秒 |
| e2e (shard 2/4) | 3 分 52 秒 |
| e2e (shard 4/4) | 3 分 47 秒 |
| e2e (shard 3/4) | **4 分 14 秒**（最重） |
| e2e report（統合） | 27 秒 |

- **偏り: 1.18 倍**（最重 4 分 14 秒 / 最軽 3 分 36 秒）。判定閾値 1.5 倍を下回るため shard 数の変更は行わない。
- **統合レポート: 376 件 / 46 ファイル**（expected 376 / unexpected 0 / flaky 0）。実テスト 368 件 + `setup` 2 件 × 4 shard = 376 で、4 shard 分がそろっていることを確認。project 内訳は `setup` 8 / `chromium` 366 / `chromium-auth` 2。
- artifact は `blob-report-{1..4}-1`（retention 1 日）と統合済み `playwright-report`（retention 30 日）が生成された。

#### 想定 3 分 12 秒との差（1 分 38 秒）の内訳

shard 1 の実測内訳は準備 68 秒 + テスト 138 秒。Issue 記載の準備 68 秒と完全に一致した一方、テストが理想値 124 秒（495 ÷ 4）に届かなかった。

`495 = F + V` と `138 = F + V/4` から **shard 内の固定費 F ≈ 19 秒**（dev server 起動 + JIT コンパイル）、可変部 V ≈ 476 秒。当初想定はこの F と、統合ジョブ 27 秒・post ステップ・偏り（平均でなく最重 shard が律速）を勘定に入れていなかった。

#### shard 数を 6 に上げた場合の試算（今回は見送り）

上のモデルで N=6 なら shard あたりのテストは 19 + 476/6 ≈ 98 秒、最重 shard で約 3 分 8 秒、統合込みで **約 3 分 45 秒**（現状比 約 1 分短縮）。ランナー時間は 1.5 倍になる。閾値判定では変更不要のため据え置き、必要になった時点で matrix の配列 1 行で切り替える。
