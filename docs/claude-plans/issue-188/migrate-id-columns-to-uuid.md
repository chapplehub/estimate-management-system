...
### 1. FK制約の扱い: 計画Aではなく計画Bを初手で採用
- **計画**: 推奨Aの「ALTER TYPEのみ（FK維持）」を先に試し、失敗時にBへフォールバック
- **実際**: `prisma migrate dev --create-only` が生成したSQLにFK制約のDROP/ADDが含まれており、制約名も正確に取得できたため、最初からフェーズB（FK制約DROP → ALTER TYPE → FK制約再ADD）で実装した
- **理由**: Prisma自身がFK維持のままの型変換をサポートしていないことが自動生成SQLから明らかだったため、Aを試すリスクを回避

### 2. テスト内のハードコードIDの修正が追加作業として発生
- **計画**: 「アプリケーションコードの変更は不要」と記載。テスト内の非UUID文字列IDの存在を見落としていた
- **実際**: 22ファイルのテストで `"non-existent-id"` や `"test-update-id-001"` 等の非UUID形式文字列をIDとして使用しており、UUID型カラムが拒否したため修正が必要だった
- **対応**: 有効なUUID形式（`"00000000-0000-7000-8000-000000000000"` 等）に置換し、別コミットとして記録
- **教訓**: TEXT→UUID型移行ではテストデータのハードコードIDも調査対象に含めるべき
