...
## 5. バックフィルのサロゲート id は `gen_random_uuid()`（v4）に確定

- **元の計画**: Step 9 で「PG バージョンを実装時に確認し `uuidv7()`／uuidv7 SQL 関数／
  `gen_random_uuid()` を選ぶ」と未確定。
- **実際の実装**: `gen_random_uuid()`（v4）に確定。
- **逸脱の理由**: 移行は一回限りでサロゲートは差分 upsert の identity に過ぎず時系列性が
  不要。PG バージョン分岐を避け移行 SQL を決定的に保てる（ADR が一回限りに限り v4 許容）。
